#!/usr/bin/env bash
#
# Build a distributable BrewBrowser.app and zip it for the Homebrew cask.
#
...
# Output: dist/BrewBrowser-<version>.zip  + its SHA-256 (paste into the cask)
...
VERSION="$(xcodebuild -project BrewBrowser.xcodeproj -scheme "$SCHEME" -configuration "$CONFIG" \
  -showBuildSettings 2>/dev/null | awk -F' = ' '/ MARKETING_VERSION =/{print $2; exit}')"
VERSION="${VERSION:-0.0.0}"

echo "==> Building $SCHEME $VERSION ($CONFIG)"
xcodebuild -project BrewBrowser.xcodeproj -scheme "$SCHEME" -configuration "$CONFIG" \
  -derivedDataPath "$DERIVED" clean build | tail -5

APP="$DERIVED/Build/Products/$CONFIG/BrewBrowser.app"
...
echo "==> Zipping"
mkdir -p "$DIST"
ZIP="$DIST/BrewBrowser-$VERSION.zip"
rm -f "$ZIP"
...
echo "  2. In Casks/brewbrowser.rb set version \"$VERSION\" and sha256 \"$SHA\"."
...
#     --sign "$DEV_ID" "$APP"
#   /usr/bin/ditto -c -k --keepParent "$APP" "$ZIP"
#   xcrun notarytool submit "$ZIP" --keychain-profile "BrewBrowserNotary" --wait
#   xcrun stapler staple "$APP"
#   /usr/bin/ditto -c -k --keepParent "$APP" "$ZIP"   # re-zip with stapled ticket
#
# (Store creds once: xcrun notarytool store-credentials "BrewBrowserNotary"
